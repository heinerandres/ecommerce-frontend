...
export default function FormEditarCategoria({categoria} : Props) {
    ...
    const handleEditar = async(e:any) => {
        e.preventDefault();
        const respuestaEditar = await editarCategoria({_id: categoria?._id, nombre});
        if (respuestaEditar.ok) router.push('/admin/categorias');
        ...
        <>
            ...
            <form onSubmit={handleEditar} className="flex flex-col border border-gray-200 shadow-lg p-10 rounded-2xl">
                ...
                <input
                    minLength={2}
                    required
                    className="px-5 md:text-sm 2xl:text-lg md:py-2 2xl:py-2 bg-white border rounded mb-3"
                    type="text" 
                    placeholder="Nombre"
                    name="nombre"
                    value={nombre}
                ...
                />
                <p className="text-xs text-gray-500">Editar la categoría para organizar los productos.</p>
                { errorMsg !== "" &&
