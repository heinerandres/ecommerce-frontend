...
export const getGeneralInformation = async(slug: string | null = null, conProductos=true) => {
  ...
  try {
    ...
    if (conProductos){
      if(slug){
        ...
        if (respuestaProductos.ok) {
            productos = respuestaProductos.producto;
        } else {
            errorMsg = respuestaProductos.msg;
        }
    }
    else{
    ...
    if (respuestaTallas.ok) {
      tallas = respuestaTallas.talla;
    } else {
      errorMsg = respuestaTallas.msg;
    }
